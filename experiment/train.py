...
from fgvc.datasets import get_dataloaders, get_dataloaders_contrastive
from fgvc.losses import FocalLossWithLogits, RecallatKSurrogate, SeesawLossWithLogits
...
def train_clf(
    ...
    **kwargs,
    ...
    elif config["loss"] == "RecallatKSurrogate":
        criterion = RecallatKSurrogate(
            sigmoid_temperature=0.01,
            batch_size=config["batch_size"],
            samples_per_class=4,
            k_values=[1, 2, 4, 8, 16],
            k_temperatures=[1, 2, 4, 8, 16],
        )
    else:
    ...
    train(
        ...
        mini_batch_size=config.get("mini_batch_size", None)
    )
