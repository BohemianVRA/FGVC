...
def get_optimizer(name: str, params: Iterable, lr: float, *args, **kwargs) -> Optimizer:
    ...
    if name == "adamw":
        ...
        raise ValueError(f"Argument 'name' should be either 'adamw' or 'sgd'.")
    return optimizer


def reduce_lr_on_plateau(optimizer: Optimizer, *args, **kwargs):
    return ReduceLROnPlateau(optimizer, 'min', factor=0.9, patience=1, verbose=True, eps=1e-6)


def cosine_lr_scheduler(optimizer: Optimizer, epochs: int, cycles: int = 5, *args, **kwargs):
...
def cosine_annealing_lr(optimizer: Optimizer, epochs: int, *args, **kwargs):
    return CosineAnnealingLR(optimizer, T_max=epochs, eta_min=0)


def get_scheduler(name: str, optimizer: Optimizer, *args, **kwargs):
    name = name.lower()
    if name == "plateau":
        ...
        raise ValueError(
            f"Argument 'name' should be either 'plateau', 'cyclic_cosine', or 'cosine'."
        )
    return scheduler
